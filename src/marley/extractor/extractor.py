...
def _normalize_unicode(text: str) -> str:
    ...
    PDF documents commonly use typographic quotes, dashes, and other
    special characters.  Normalising them to ASCII improves downstream
    search consistency while preserving readability.
    """
...
def _cell_text(cell: str | None) -> str:
    ...
    if cell is None:
        return ""
    return _normalize_unicode(cell.replace("\n", " ").strip())
...
def _is_header_row(row: list[str | None]) -> bool:
    ...
    return (
        "Name of module" in joined
        or "German translation" in joined
        or "Deutscher Modultitel" in joined
    )
...
def _is_section_label_row(row: list[str | None]) -> bool:
    ...
    Section labels always appear in the first column (index 0). Rows where
    the only non-empty cell is in a later column are cross-page continuation
    rows whose text belongs to the previous module row.
    """
    non_empty = [c for c in row if c and c.strip()]
    if len(non_empty) == 1:
        text = non_empty[0].strip()
        # Section labels appear in column 0; if column 0 is empty, this is
        # a continuation row, not a section label.
