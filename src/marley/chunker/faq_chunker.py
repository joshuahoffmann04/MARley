...
from src.marley.models.constants import DEFAULT_MAX_CHUNK_TOKENS, DEFAULT_TOKENIZER
...
def chunk_faq(
    ...
    *,
    ...
    for i, entry in enumerate(dataset.entries):
        ...
        if token_count > DEFAULT_MAX_CHUNK_TOKENS:
            flags.append(QualityFlag(
                code="FAQ_OVERSIZED_ENTRY",
                message=f"Entry '{entry.id}' has {token_count} tokens (>{DEFAULT_MAX_CHUNK_TOKENS}).",
                severity="info",
