...
Abstract base class for all retrieval strategies. Concrete implementations (`BM25Retriever`, `VectorRetriever`, `HybridRetriever`, `FusionRetriever`) are defined in `src/marley/retrieval/`.
...
| `context` | List of chunk dicts, each with `chunk_id`, `text`, and optionally `metadata`. |
...
class GenerationResult:
    ...
    context_chunk_ids: list[str] = []  # Chunk IDs used as context
