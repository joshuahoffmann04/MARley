...
them into a uniform [0, 1] range so that a single threshold applies
across all retrieval strategies. Normalized confidences are **not
comparable across retriever types** — the same threshold value has
different effects depending on the retriever.

For normalization formulas, examples, and practical implications, see
[Score Normalization](../retrieval/overview.md#score-normalization).

### Threshold Filtering

After normalization, the retrieval confidence is computed as the maximum
normalized score across all results (before filtering):

```
confidence = max(normalized_scores)  # computed before filtering
```

Passages with a normalized score below the configured threshold are
then discarded. If no passages survive filtering, the pipeline abstains
at Level 1 without invoking the generator. The confidence value is
preserved in the `AbstentionResult` regardless of whether abstention
occurs.
...
The module defines a constant `ABSTENTION_PREFIX = "ABSTENTION:"` and two functions.
...
class AbstentionResult:
    ...
    Each dict has 'chunk_id', 'text', 'score', and 'metadata'."""
...
└── abstention/
    ├── __init__.py                Exports: ABSTENTION_PREFIX, detect_abstention, extract_abstention_reason
    ├── confidence.py              Re-export of models.scoring functions (backward compatibility)
    ├── detection.py               LLM output abstention detection
    └── pipeline.py                Re-export of server.pipeline.run_with_abstention (backward compatibility)
```

The abstention module's public API focuses on detection. Score normalization lives in `src/marley/models/scoring` and the pipeline orchestrator lives in `src/marley/server/pipeline`. The `confidence.py` and `pipeline.py` files in the abstention package are thin re-export modules for backward compatibility.
...
- [Retrieval Overview](retrieval/overview.md) — Retriever architecture
  and score normalization context.
