...
This document defines every JSON file format produced and consumed by the MARley pipeline. For the Python data classes that represent these structures in code, see [models.md](models.md).
  ...
  "chunks": [
    {
      ...
      "metadata": {
        ...
        "parent_section_id": "part-III",
        "heading_path": [
          "III. Examination-related provisions",
          "§23 Master's Thesis"
        ],
        "start_page": 11,
        "end_page": 13,
...
## 5. Evaluation Datasets (`evaluation.json`, `evaluation-*.json`)

All evaluation files share **the same schema**. The master file (`evaluation.json`) contains 100 questions used to evaluate the full MARley pipeline. These questions are **intentionally worded differently** from the FAQ entries to test genuine retrieval and generation quality rather than exact matching. The annotated variants (`evaluation-*.json`) are per-knowledge-base copies with `relevant_chunks` populated by manual annotation, used for retrieval evaluation.

### Files

| File                       | `knowledge_base` | Description                                                           |
| -------------------------- | ---------------- | --------------------------------------------------------------------- |
| `evaluation.json`          | `"master"`       | Master dataset. `relevant_chunks` is empty for all questions.         |
| `evaluation-stpo.json`     | `"stpo"`         | Annotated for StPO chunks (153 text + table). 75 annotated questions. |
| `evaluation-faq-stpo.json` | `"faq-stpo"`     | Annotated for FAQ-StPO chunks (1039 Q/A). 75 annotated questions.     |
| `evaluation-faq-ao.json`   | `"faq-ao"`       | Annotated for FAQ-AO chunks (0 Q/A, placeholder). 21 annotated questions. |
...
| `metadata.knowledge_base`         | string  | Identifier of the knowledge base: `"master"`, `"stpo"`, `"faq-stpo"`, or `"faq-ao"`.                                                |
...
| `questions[].relevant_chunks`     | array   | List of chunk IDs needed to answer this question. Empty in the master file; populated per knowledge base in the annotated variants. |
...
└── testing/
    ...
    ├── evaluation-stpo.json              # Annotated for StPO chunks (Section 5)
    ├── evaluation-faq-stpo.json          # Annotated for FAQ-StPO chunks (Section 5)
    └── evaluation-faq-ao.json            # Annotated for FAQ-AO chunks (Section 5)
```
