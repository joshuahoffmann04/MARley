...
class TestFAQStPOChunking:
    def test_total_chunks(self, stpo_result):
        assert stpo_result.stats.total_chunks == 1039
...
ao_skip = pytest.mark.skipif(not FAQ_AO_PATH.exists(), reason="faq-ao.json not found")
...
class TestFAQAOChunking:
    def test_placeholder_produces_no_chunks(self, ao_result):
        assert ao_result.stats.total_chunks == 0
        assert ao_result.stats.entries_skipped == ao_result.stats.entries_total

    def test_placeholder_raises_all_skipped(self, ao_result):
        codes = [f.code for f in ao_result.quality_flags]
        assert "FAQ_ALL_SKIPPED" in codes
