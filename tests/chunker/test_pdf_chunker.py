...
class TestMergeUndersized:
    ...
    def test_single_chunk(self):
        result = _merge_undersized(
            ["Only one."], ENCODER, min_tokens=20, max_tokens=200
        )
...
class TestBuildHeadingPrefix:
    ...
    def test_appendix(self):
        appendix = _section("appendix-2", "appendix", "Appendix 2", "Module List")
...
class TestBuildTableChunks:
    def test_small_table_single_chunk(self):
        ...
        result = _build_table_chunks(
            table, ENCODER, max_tokens=200, heading_prefix=None
        )
        assert len(result) == 1
...
class TestChunkId:
    ...
    def test_table_chunk_id_format(self):
        table = Table(
            table_id="par-1-tbl-1", page=1, headers=["A", "B"], rows=[["x", "y"]]
        )
        ...
        for sid in multi:
            ...
            for i in range(len(chunks) - 1):
                ...
                if not overlap_found:
                    ...
                    assert (
                        chunks[i].token_count > 400 or chunks[i + 1].token_count > 400
                    ), (
    ...
    def test_all_sections_produce_chunks(self, extraction, chunking_result):
        ...
        for section in extraction.sections:
            if section.text.strip() or section.tables:
                assert (
                    section.section_id in chunked_sections
...
class TestHeadingPaths:
    def test_paragraph_includes_part_in_path(self, chunking_result):
        par_chunks = [
            c for c in chunking_result.chunks if c.metadata.section_kind == "paragraph"
        ]
        assert par_chunks
        for chunk in par_chunks:
            ...
            assert (
                has_part
            ), f"Paragraph chunk missing part in path: {chunk.metadata.heading_path}"

    def test_appendix_has_correct_path(self, chunking_result):
        app_chunks = [
            c for c in chunking_result.chunks if c.metadata.section_kind == "appendix"
        ]
        assert app_chunks
    ...
    def test_preamble_path(self, chunking_result):
        preamble_chunks = [
            c for c in chunking_result.chunks if c.metadata.section_id == "preamble"
        ]
