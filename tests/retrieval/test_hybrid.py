...
class TestHybridRetrieverUnit:
    ...
    def test_weights_boost_second_retriever(self):
        r1 = _FakeRetriever({"q": [("c1", 1.0)]})
        r2 = _FakeRetriever({"q": [("c2", 0.9)]})
        hybrid = HybridRetriever(retrievers=(r1, r2), weights=[1.0, 2.0])
        results = hybrid.retrieve("q", k=2)
        assert results[0].chunk_id == "c2"

    def test_uniform_weights_match_default(self):
        r1 = _FakeRetriever({"q": [("c1", 1.0), ("c2", 0.5)]})
        r2 = _FakeRetriever({"q": [("c2", 0.9), ("c3", 0.4)]})
        default = HybridRetriever(retrievers=(r1, r2))
        explicit = HybridRetriever(retrievers=(r1, r2), weights=[1.0, 1.0])
        res_d = default.retrieve("q", k=3)
        res_e = explicit.retrieve("q", k=3)
        for rd, re in zip(res_d, res_e):
            assert rd.chunk_id == re.chunk_id

    def test_weights_wrong_length_raises(self):
        r = _FakeRetriever()
        with pytest.raises(ValueError, match="Expected 2 weights"):
            HybridRetriever(retrievers=(r, r), weights=[1.0])
