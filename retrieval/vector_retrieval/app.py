...
from retrieval.vector_retrieval.service import VectorBackendUnavailableError, VectorRetriever

settings = get_vector_retrieval_settings()
runtime_config = get_vector_retrieval_config(settings)
retriever = VectorRetriever(settings=settings, runtime_config=runtime_config)

app = FastAPI(
    title=f"{settings.app_name} - Vector Retrieval (ChromaDB)",
    description="Vector retrieval service for MARley.",
)
